...
export const isFileValid = async (uploadPath: string, name: string) => {
	const stream = fs.createReadStream(`../client/public/uploads/${uploadPath}/${name}`);
	...
	if (!isValid) {
		fs.unlink(`../client/public/uploads/${uploadPath}/${name}`, (err) => { });
		return undefined;
	}
	return name;
}
...
export const resizeAvatar = async (uploadPath: string, name: string) => {
	const ext = path.extname(name);
	const image = await Jimp.read(`../client/public/uploads/${uploadPath}/${name}`);
	const resizeName = name.slice(0, name.indexOf(ext));
	image.resize(220, 220).write(`../client/public/uploads/${uploadPath}/${resizeName}x220${ext}`);
	image.resize(70, 70).write(`../client/public/uploads/${uploadPath}/${resizeName}x70${ext}`);
