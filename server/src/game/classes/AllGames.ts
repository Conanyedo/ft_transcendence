...
export class allGames {
	...
	joinRankLobby(client: Socket, login: string) {
		const inRankGame = this.RankGames?.find(
				(game) => game._PlayerLeft?.getlogin() === login || game._PlayerRight?.getlogin() === login);
		const inFriendGame = this.FriendGames?.find(
				(game) => game._PlayerLeft?.getlogin() === login || game._PlayerRight?.getlogin() === login);
		if (inRankGame || inFriendGame || (this.RankLobby.length && this.RankLobby[0]?.login === login)) {
			client.emit('errorCheck', 'in game already');
            console.log(inRankGame?._ID, inFriendGame?._ID);
			return;
			}
		this.RankLobby.push(new Lobby(login, '', client));
		if (this.RankLobby.length > 1 && this.RankLobby[0].login !== this.RankLobby[1].login) {
	...
	removeGame(gameID: string) {
		...
		if (oldFriendLength !== this.FriendGames.length || oldRankLength !== this.RankGames.length) {
				this.countLiveGames--;
				this.liveGames();
		}
	}
	...
	clientDisconnect(client: Socket) {
		if (this.RankGames.length) {
			const game = this.RankGames.find(game => game._PlayerLeft.getsocket().id === client.id || game._PlayerRight.getsocket().id === client.id)
			if (game)
				game.pausegame(this.server, this);
		}
		if (this.FriendGames.length) {
			const game = this.FriendGames.find(game => game._PlayerLeft.getsocket().id === client.id || game._PlayerRight.getsocket().id === client.id)
			if (game)
				game.pausegame(this.server, this);
		}
		if (this.RankLobby.length) {
