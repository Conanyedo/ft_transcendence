				...
				<motion.div ref={ref_nav} className={classes.sideNavCtn}>
					<motion.div className={`${classes.sidenav}`}>
						<div className={`${classes.sideItems}`}>
							{NAVITEMS.map((item) => (
								<ItemsNav key={item.alt} alt={item.alt} src={item.src} ref_ctn={item.ref_ctn} />
							))}
						</div>
						<div className={classes.sideItems}>
							<div className={`${classes.backIcons} ${classes.logOut}`} onClick={handlerLogOut}>
								<Image src={NamePage === "/logout" ? LogoutSelected : Logout} width={79} height={79} />
							</div>
						</div>
					</motion.div>
					<div className={classes.sidenavInd}>
						<motion.div className={`${props.currentPos} ${classes.Indicator}`} animate="animate">
