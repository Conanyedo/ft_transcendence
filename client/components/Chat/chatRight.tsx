...
const SendMsg: React.FunctionComponent<{
	...
	enteredMsg: string
	setEnteredMsg: any
}> = ({ currentUser, relation, stopUsr, setStopUsr, enteredMsg, setEnteredMsg }) => {
	...
	return (
		<>
			{settingGames && <SettingGame sendGame={sendGame} login={currentUser?.login} Hide={HideSetting} />}
			<div className={Styles.sendDiv} style={{ gap: enteredMsg != "" ? "1.5rem" : "0" }}>
				...
				{["banned", "muted"].includes(stopUsr) && currentUser.type != "Dm" && (
					<div className={Styles.msgInput}>
						<div className={Styles.newCnv}>You were {stopUsr} from this channel</div>
					</div>
				)}
				{stopUsr == "" && relation != "Blocker" && (
					<div
										...
										{(!chatMsg.invitation && (
											<>
												<div
													...
													style={{
														backgroundColor: chatMsg.sender == me ? "#409CFF" : "#3A3A3C",
														borderRadius:
...
const THeader: React.FunctionComponent<{
	...
	setRelation: any
	setShowSetModal: any
}> = ({ profile, setShowprofile, currentUser, relation, setRelation, setShowSetModal }) => {
	// context here
	...
	return (
		<div className={Styles.topDetails}>
			<div className={Styles.flex}>
				...
				{profile && (
					<div className={Styles.backArrowProfile} onClick={() => setShowprofile(false)}>
						<BackArrow />
					</div>
				)}
				...
				<div
					...
					<div>
						...
						<p className={Styles.chatUserStatus}>
							{currentUser?.membersNum ? currentUser?.membersNum + " members" : currentUser?.status}
						</p>
					</div>
				</div>
			</div>
...
export const ChatRight = (props: { setShowSetModal: any; setSelectedConv: any; login: number }) => {
	...
	useEffect(() => {
		if (currentUser?.convId == undefined && currentUser?.relation == "friend") setShowCnv(true)
		setConvStatus(currentUser, setStopUsr)
		setConvId(currentUser?.convId)
		setRelation(currentUser?.relation)
	}, [currentUser])
