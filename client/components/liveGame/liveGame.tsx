...
import { EmtyUser, headerDataType, headerGameType, UserTypeNew } from "../../Types/dataTypes"
...
export const LiveGame: React.FC<headerGameType> = (props) => {
	...
	useEffect(() => {
		if (Data?.firstPlayer) fetchDATA(setUserInfo1, router, `user/info/${Data?.firstPlayer}`)
		if (Data?.secondPlayer) fetchDATA(setUserInfo2, router, `user/info/${Data?.secondPlayer}`)
	}, [Data?.firstPlayer])
				...
				<div className={classes.GameContent}>
					<div className={classes.UserSection}>
						<div className={classes.userNameAvatar} onClick={goToFirstProfile}>
							...
							<div className={classes.userName}>{userInfo1.fullname.split(" ")[0]}</div>
						</div>
						<div className={classes.ScoreTier}>
							<div className={classes.tierContainer}>
								<Image src={rank1.tier} />
							</div>
							<div className={classes.Score}>{Data?.firstScore}</div>
						</div>
					</div>
					<div className={classes.vs}>VS</div>
					<div className={classes.UserSectionSecond}>
						<div className={classes.ScoreTier}>
							<div className={classes.Score}>{Data?.secondScore}</div>
						...
						<div className={classes.userNameAvatar} onClick={goToSecondProfile}>
							<div className={classes.secondUserName}>{userInfo2.fullname.split(" ")[0]}</div>
...
const LiveGames = () => {
	const [liveMatchs, setLiveMatchs] = useState<headerGameType[]>([])
	useEffect(() => {
		...
		<>
			<div className={classes.LivegamesContainer}>
				...
				<div className={classes.listLiveGames}>
					{(liveMatchs.length > 0 &&
						liveMatchs?.map((game) => {
							return <LiveGame key={game.gameId} click={true} gameId={game.gameId} />
						})) || <p>No Live Game now</p>}
				</div>
			</div>
		</>
	)
