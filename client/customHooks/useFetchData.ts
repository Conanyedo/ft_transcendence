...
export const getLoginInfo = async (login: any, user: boolean, setChnlData: any) => {
	const token = getCookie("jwt")
...
export const check2FA_JWT = async (jwt: CookieValueTypes, set: any, router: NextRouter) => {
	await axios({
		method: "get",
...
export const checkJWT = async (router: NextRouter, set: Dispatch<SetStateAction<boolean>>) => {
	const token = getCookie("jwt")
	await axios({
		method: "get",
...
export const requestsChannel = async (convId: string, path: string, router: NextRouter) => {
	const token = getCookie("jwt")
	const json = JSON.stringify({ convId: convId })
	return await axios({
		method: "post",
