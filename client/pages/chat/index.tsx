...
const Chat = () => {
	...
	const updateConversations = (msgConvId: string) => {
		if (msgConvId) fetchDATA(setNewConv, router, `chat/conversations/${msgConvId}`)
	}
    ...
    const convId = getSelectConvId()
		...
		const loginInfo = convId !== "0" ? await fetchLoginInfo(convId) : undefined
		if (loginInfo) setConvData({ name: loginInfo.fullname, type: "Dm", ...loginInfo })
	}
	...
	useEffect(() => {
		...
		if (newConv.convId !== "0") {
			...
			if (newConv.status === "Blocker" || (oldConv?.status === "Blocker" && newConv.status !== "Blocker")) {
				convs.map((conv) => {
	...
	useEffect(() => {
		...
		if (convs.length > 0 && convId !== "0" || convId !== "0") {
			let foundconv = convs.find((conv) => conv.convId === convId || conv.login === convId)
